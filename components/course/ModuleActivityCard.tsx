...
export function ModuleActivityCard({
  ...
  title: string;
  description: string;
  /** Always shown — every activity card states its status explicitly (never blank), per spec. */
  status: { complete: boolean; label?: string };
}) {
  const statusText = status.complete ? (status.label ?? "Completed") : (status.label ?? "Not completed");
  return (
    <Link
      href={href}
      ...
      <div>
        ...
        <p
          className={`mt-1.5 inline-flex items-center gap-1.5 text-xs font-semibold ${
            status.complete ? "text-brand-700" : "text-ink-400"
          }`}
        >
          {status.complete ? <IconCheckCircle className="h-3.5 w-3.5" /> : null}
          Status: {statusText}
        </p>
      </div>
